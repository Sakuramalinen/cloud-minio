...
public class UploadTaskRecordServiceImpl extends ServiceImpl<UploadTaskRecordMapper, UploadTaskRecord> implements IUploadTaskRecordService {
    ...
    @Override
    public UploadTaskRecord createUploadTaskRecord(CreateUploadTaskRecordDTO dto) {
        Long userId = UserContext.getUser();
        ...
        //判断该目录是否有重复文件
        UserFile userFile = fileNameExist(dto.getParentId(), dto.getFileName());
        if (userFile != null) {
        ...
        //获取对象存储路径
        String objectPath = fileUtils.getObjectStorePath(dto.getFileMd5(), dto.getFileName());
