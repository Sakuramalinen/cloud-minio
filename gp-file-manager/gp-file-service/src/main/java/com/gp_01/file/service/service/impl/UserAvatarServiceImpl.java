...
public class UserAvatarServiceImpl extends ServiceImpl<UserAvatarMapper, UserAvatar> implements IUserAvatarService {
    ...
    public String uploadAvatar(String filename) {
        ...
        String url = minioUploader.getWholeUploadUrl(minioConfig.getBucketName(), avatarFileStorePath, 5, TimeUnit.MINUTES);
        //将地址存到内存
    ...
    public String previewAvatar(Long id) {
        ...
        return minioPreviewer.getPreviewPreSignedUrl(minioConfig.getBucketName(), one.getObjectPath(), one.getContentType(), 1, TimeUnit.DAYS);
    }
    ...
    public List<ListHistoryAvatarVO> previewHistoryAvatarList() {
        ...
        for (UserAvatar userAvatar : list) {
            //获取预签名url
            String url =  minioPreviewer.getPreviewPreSignedUrl(minioConfig.getBucketName(), userAvatar.getObjectPath(), userAvatar.getContentType(), 1, TimeUnit.DAYS);
    ...
    public Long persistenceAvatar() {
        ...
        try {
            contentType = fileUtils.getContentTypeByFileBinary(objectPath);
             size = minioUtils.getFileStatus(minioConfig.getBucketName(), objectPath).getSize();
        }catch(Exception e){
